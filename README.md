...
No marketplace WARP action runs in the crawl job. A short checked-in script installs only Ubuntu-archive networking tools, brings up a fixed kernel WireGuard profile, verifies the WARP route, and exits **before** the law.go.kr API key is injected into the crawler step. The profile private key is not stored in this repository: `WARP_PRIVATE_KEY` must be configured as a GitHub Actions secret and is exposed only to the WARP step. R2 and Telegram credentials exist only in separate trusted steps; production Python dependencies, including transitives, are exact-version and wheel-hash locked, and the Docker base image is digest-pinned. Residual trust is explicit: the GitHub-hosted runner, pinned official GitHub Actions, Ubuntu package archive, Python wheel artifacts, and Cloudflare WARP service remain in the trusted computing/network base. The fixed WARP identity can still suffer endpoint roaming, rate limiting, or revocation, which can stop the crawler. The durable replacement is a controlled Korean/allowed-egress ephemeral runner or a minimal law.go.kr relay with job-scoped authentication; adopting either requires external infrastructure and credential authority outside this repository. Until then, route verification fails closed and WARP/law.go.kr 403/429 failures require operational monitoring. The trusted finalize job checks the SQLite state, raw/gzip equality, payload size and full record schema before publication, but it cannot independently prove the semantic correctness of every upstream public record.
