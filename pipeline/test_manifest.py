...
[명세 계약]
  build_manifest(core_bytes, tax_bytes=None, built_at=None) -> dict
    ...
    - total은 payload의 'total' 필드가 아니라 len(cases) — 클라이언트가
      IndexedDB count()와 직접 비교하는 값이므로 키 수가 정답.
...
class TestBuildManifest(unittest.TestCase):
    ...
    def test_tax_제공시_같은_스키마로_포함(self):
        ...
        m = build_manifest(make_payload(), tax_bytes=tax)
